...
void Sema::MarkTHTableUsed(SourceLocation Loc, CXXRecordDecl *Class,
                           bool DefinitionRequired) {
  Class = cast<CXXRecordDecl>(Class->getCanonicalDecl());
  std::pair<llvm::DenseMap<CXXRecordDecl *, bool>::iterator, bool>
    Pos = THTablesUsed.insert(std::make_pair(Class, DefinitionRequired));
  ...
  if (Class->isLocalClass()) {
    ...
    THTableUses.push_back(std::make_pair(Class, Loc));
  }
}

bool Sema::DefineUsedTHTables() {
  if (THTableUses.empty())
    return false;

  bool DefinedAnything = false;

  // for (unsigned I = 0; I != THTableUses.size(); ++I) {
  //   CXXRecordDecl *Class = THTableUses[I].first->getDefinition();
  //   if (!Class)
  //     continue;

  //   // SourceLocation Loc = THTableUses[I].second;
  //   DefinedAnything = true;

  //   CXXRecordDecl *Canonical = cast<CXXRecordDecl>(Class->getCanonicalDecl());
  //   Consumer.HandleTHTable(Class, THTablesUsed[Canonical]);
  // }
  return DefinedAnything;
}
